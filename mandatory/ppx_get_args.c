...
/*   Updated: 2023/05/13 15:11:57 by acomet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ppx_get_args(char *cmd, char *cmd_full)
{
	...
	count = ppx_flags_args_count(cmd_full);
	...
	if (count)
	{
		if (ppx_get_flags_to_args(args, cmd_full))
			return (NULL);
	}
	args[count + 1] = NULL;
	return (args);
...
int	ppx_get_flags_to_args(char **args, char *cmd_full)
{
	...
	while (cmd_full[i])
	{
		...
		if (cmd_full[i])
		{
			args[j] = ppx_strdup_one_flag_arg(cmd_full + i);
			if (!args[j])
				return (1);
			j++;
		}
...
char	*ppx_strdup_one_flag_arg(const char *s)
{
	...
	while (s[i] && s[i] != ' ')
		i++;
	if (s && s[0] == '\'' && s[i - 1] == '\'' && i != 1)
		return (ppx_strdup_rm_simple_quotes(s));
...
char	*ppx_strdup_rm_simple_quotes(char const *s)
{
	char	*str;
	int		i;
	int		j;
